...
if (isset($_POST['btn_DatTour'])) {
    ...
    $succhua_new = $succhua - $sl;
    $giamgia = $rows["GiamGia"];
    if($giamgia > 0) {
        $percent = $giamgia / $giaNguoiLon;
        $TongTien = ($giamgia * $soNguoiLon) + (($giaTreEm * $percent) * $soTreEm);
    }
    else {
    ...
    if ($query_run) {
        ...
        if ($query2_run) {
            echo '<script>alert("Đặt tour thành công! Vui lòng kiểm tra Email");</script>';
        ...
        <body>
            ...
            if ($mail->Send()) {
                $sql_succhua = "UPDATE tourdulich SET SucChua = $succhua_new WHERE MaTour = $matour";
                $query_succhua = mysqli_query($connection, $sql_succhua);

                if ($query_succhua) {
                    echo "<div class='alert alert-success text-center'>Đặt Tour thành công.</div>";
                } else {
                    echo "<div class='alert alert-danger'>Đặt Tour tất bại.</div>";
...
<script type="text/javascript">
    function giaTreEm() {
        if (<?php echo $rows["GiamGia"] ?> > 0) {
            <?php
            $percent = ($rows["GiamGia"] / $rows["GiaTien"]);
            ?>
            var sum = document.getElementById("child").value * <?php echo $rows["GiaTreEm"] * $percent ?>;
        } else {
    ...
    function giaNguoiLon() {
        if (<?php echo $rows["GiamGia"] ?> > 0) {
            var sum = document.getElementById("adult").value * <?php echo $rows["GiamGia"] ?>;
        } else {
    ...
    function tongTien() {
        if (<?php echo $rows["GiamGia"] ?> > 0) {
            ...
            $percent = ($rows["GiamGia"] / $rows["GiaTien"]);
            ?>
            var sum = (document.getElementById("child").value * <?php echo $rows["GiaTreEm"] * $percent ?>) + (document.getElementById("adult").value * <?php echo $rows["GiamGia"] ?>);
        } else {
    ...
    function soHanhKhach() {
        ...
        if ($("#adult").val() == "0") {
            ...
            if ($("#adult").val() == "" || $("#child").val() == "") {
                ...
                if (x > <?php echo $succhua ?>) {
                    alert("Số người vượt quá số lượng cho phép");
                } else {
                    for (var i = 1; i <= x; i++) {
    ...
    <div class="row">
        <script>
            $("#form-dat-tour").validate({
                ...
                messages: {
                    SoNguoiLon: {
                        required: 'Số người lớn khống được để trống'
                    }
                }
            })
            ...
            <form id="form-dat-tour" name="myForm" method="post">
                <div class="tab-content" id="nav-tabContent">
                    ...
                    <div class="tab-pane fade" id="nav-tab-2" role="tabpanel" aria-labelledby="nav-profile-tab">
                        ...
                        <button type="button" class="btn btn-previous" id="btn_tab2-previous" onclick="tab2Pre(), ResetThanhVien()">Trở Về</button>
